...
let pendingCache=[];async function loadPending(){pendingCache=await api('pending')||[];$('navPending').textContent=pendingCache.length;const short=pendingCache.slice(0,5);$('urgentList').innerHTML=short.length?short.map(p=>`<div class="urgent-item"><div class="urgent-avatar">${initials(p.customer_name)}</div><div><h4>${esc(p.customer_name)} · ${esc(p.reason||'Ação humana')}</h4><p>${esc(p.summary||p.primary_service_name||'Sem resumo')} · ${ago(p.created_at)}</p></div><span class="priority ${String(p.priority).toLowerCase()==='alta'?'high':'normal'}">${esc(p.priority||'normal')}</span></div>`).join(''):empty('Nenhuma pendência aberta.');$('pendingCards').innerHTML=pendingCache.length?pendingCache.map(p=>`<article class="pending-card"><div class="pending-top"><span class="priority ${String(p.priority).toLowerCase()==='alta'?'high':'normal'}">${esc(p.priority||'normal')}</span><small>${ago(p.created_at)}</small></div><h3>${esc(p.customer_name)}</h3><p>${esc(p.summary||p.reason||'Aguardando ação humana.')}</p><div class="pending-info"><div><span>Serviço</span><strong>${esc(p.primary_service_name||p.metadata?.collected?.servico||'—')}</strong></div><div><span>Status</span><strong>${esc(p.status)}</strong></div></div><div class="pending-actions"><button class="btn secondary" data-open-contact="${esc(p.contact_key)}">Ver conversa</button></div></article>`).join(''):empty('A Iana não tem nenhum handoff pendente para Nayara.');document.querySelectorAll('[data-open-contact]').forEach(b=>b.onclick=async()=>{gotoPage('conversas');await loadConversations('',b.dataset.openContact)})}
